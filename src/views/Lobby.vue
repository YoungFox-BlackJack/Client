      ...
      <div v-for="(room, index) in rooms" :key="index" class="col-3">
        ...
        >
          <div>Creator:
            <br>
            <b>
              <i>{{ room.player1.name }}</i>
            </b>
          </div>
          <div v-if="!room.player2 || !room.player2.name">Players: 1</div>
          <div v-else>Players: 2</div>
          <b-button
            v-if="!room.player2 || !room.player2.name"
            @click.prevent="joinRoom(room.roomId)"
            variant="primary"
          >Join Room</b-button>
      ...
      <b-row>
        ...
        <b-col>
          <form @submit.prevent="createName">
            <div class="form-group">
              <label
                style="font-size:26px;color:white; font-family: 'Charm', cursive; background-color:rgba(0,128,0,.5)"
