  ...
  <b-container class="bv-example-row">
    <b-row align-h="between">
      <b-col cols="4">
        ...
        >
          <b-button
            v-if="!player1.ready && whichPlayer == 'player1'"
            variant="primary"
            @click.prevent="changeStatus"
          >Ready?</b-button>
          <b-button
            v-else-if="!player1.ready && whichPlayer == 'player2'"
            disabled
            variant="primary"
            @click.prevent="changeStatus"
          >Ready?</b-button>
          <b-button
            v-else-if="player1.ready && whichPlayer == 'player1'"
            variant="primary"
            @click.prevent="changeStatus"
          >Ready!</b-button>
          <b-button
      ...
      <b-col cols="4">
        ...
        >
          <img
            v-if="!player2.ready==false && player2.name == ''"
            width="100px"
            src="https://thumbs.gfycat.com/WellinformedUnfoldedIndigowingedparrot-max-1mb.gif"
            alt
          >
          <h3 v-if="!player2.ready==false && player2.name == ''">Waiting . . .</h3>
          <b-button
            v-else-if="!player2.ready && player2.name != '' && whichPlayer =='player2'"
            variant="primary"
            @click.prevent="changeStatus2"
          >Ready?</b-button>
          <b-button
            v-else-if="!player2.ready && player2.name != '' && whichPlayer =='player1'"
            disabled
            variant="primary"
            @click.prevent="changeStatus2"
          >Ready?</b-button>
          <b-button
            v-else-if="player2.ready == true && whichPlayer == 'player2'"
            variant="primary"
            @click.prevent="changeStatus2"
          >Ready!</b-button>
          <b-button
            v-else-if="player2.ready == true && whichPlayer == 'player1'"
            disabled
            variant="primary"
            @click.prevent="changeStatus2"
          >Ready!</b-button>
...
export default {
  ...
  methods: {
    ...
    changeStatus() {
      if (this.player1.ready) {
        firebase
          .database()
          .ref(`rooms/${this.roomId}/player1/ready`)
          .set(false, snapshot => {});
    ...
    changeStatus2() {
      if (this.player2.ready) {
        firebase
          .database()
          .ref(`rooms/${this.roomId}/player2/ready`)
          .set(false, snapshot => {});
  ...
  mounted: function() {
    ...
    firebase
      .database()
      .ref(`rooms/${this.$route.params.roomId}`)
      .on("value", function(snapshot) {
        ...
        if (snapshot.val().playing == true) {
          self.$router.push("/play"); //change to playing route
        }
      });
  }
};
